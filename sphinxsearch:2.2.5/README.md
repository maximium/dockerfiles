...
```
docker run -it maximium/sphinxsearch:2.2.5 ls /usr/local/bin
...
```
cd dockerfiles/sphinxsearch\:2.2.5
docker build -t maximium/sphinxsearch:2.2.5 .
```

Otherwise you can pull this image from docker index.

```
docker pull maximium/sphinxsearch:2.2.5
```
...
```
docker run -i -t maximium/sphinxsearch:2.2.5 /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d maximium/sphinxsearch:2.2.5 searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d maximium/sphinxsearch:2.2.5 indexall.sh
```
...
[![Analytics](https://ga-beacon.appspot.com/UA-49657176-1/dockerfiles/sphinxsearch:2.2.5)](https://github.com/igrigorik/ga-beacon)
