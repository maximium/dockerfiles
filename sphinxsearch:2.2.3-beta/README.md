...
```
docker run -it maximium/sphinxsearch:2.2.3-beta ls /usr/local/bin
...
docker build -t maximium/sphinxsearch:2.2.3-beta .
```

Otherwise you can pull this image from docker index.

```
docker pull maximium/sphinxsearch:2.2.3-beta
```
...
```
docker run -i -t maximium/sphinxsearch:2.2.3-beta /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d maximium/sphinxsearch:2.2.3-beta searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d maximium/sphinxsearch:2.2.3-beta indexall.sh
```
