...
```
docker run -it maximium/sphinxsearch:2.2.6 ls /usr/local/bin
...
docker build -t maximium/sphinxsearch:2.2.6 .
```

Otherwise you can pull this image from docker index.

```
docker pull maximium/sphinxsearch:2.2.6
```
...
```
docker run -i -t maximium/sphinxsearch:2.2.6 /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d maximium/sphinxsearch:2.2.6 searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d maximium/sphinxsearch:2.2.6 indexall.sh
```
