...
- regular expression filter (via RE2 engine, version %%GOOGLE_RE_VERSION%%, [link](https://github.com/google/re2))
...
```
docker run -it maximium/sphinxsearch:%%SPHINX_VERSION%% ls /usr/local/bin
...
docker build -t maximium/sphinxsearch:%%SPHINX_VERSION%% .
```

Otherwise you can pull this image from docker index.

```
docker pull maximium/sphinxsearch:%%SPHINX_VERSION%%
```
...
```
docker run -i -t maximium/sphinxsearch:%%SPHINX_VERSION%% /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d maximium/sphinxsearch:%%SPHINX_VERSION%% searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d maximium/sphinxsearch:%%SPHINX_VERSION%% indexall.sh
```
...
[![Analytics](https://ga-beacon.appspot.com/UA-49657176-1/dockerfiles/sphinxsearch:%%SPHINX_VERSION%%)](https://github.com/igrigorik/ga-beacon)
