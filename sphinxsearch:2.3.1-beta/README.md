...
```
docker run -it maximium/sphinxsearch:2.3.1-beta ls /usr/local/bin
...
```
cd dockerfiles/sphinxsearch\:2.3.1-beta
docker build -t maximium/sphinxsearch:2.3.1-beta .
```

Otherwise you can pull this image from docker index.

```
docker pull maximium/sphinxsearch:2.3.1-beta
```
...
```
docker run -i -t maximium/sphinxsearch:2.3.1-beta /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d maximium/sphinxsearch:2.3.1-beta searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d maximium/sphinxsearch:2.3.1-beta indexall.sh
```
...
[![Analytics](https://ga-beacon.appspot.com/UA-49657176-1/dockerfiles/sphinxsearch:2.3.1-beta)](https://github.com/igrigorik/ga-beacon)
