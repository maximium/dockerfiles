...
```
docker run -it maximium/sphinxsearch:2.1.8 ls /usr/local/bin
...
```
cd dockerfiles/sphinxsearch\:2.1.8
docker build -t maximium/sphinxsearch:2.1.8 .
```

Otherwise you can pull this image from docker index.

```
docker pull maximium/sphinxsearch:2.1.8
```
...
```
docker run -i -t maximium/sphinxsearch:2.1.8 /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d maximium/sphinxsearch:2.1.8 searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d maximium/sphinxsearch:2.1.8 indexall.sh
```
