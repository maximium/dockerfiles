...
```
docker run -it maximium/sphinxsearch:2.2.9 ls /usr/local/bin
...
docker build -t maximium/sphinxsearch:2.2.9 .
```

Otherwise you can pull this image from docker index.

```
docker pull maximium/sphinxsearch:2.2.9
```
...
```
docker run -i -t maximium/sphinxsearch:2.2.9 /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d maximium/sphinxsearch:2.2.9 searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d maximium/sphinxsearch:2.2.9 indexall.sh
```
